...
The pipeline writes small review-facing CSV, JSON, and figure artifacts while treating large activation, circuit-matrix, token-score, and sparse-autoencoder files as reproducible generated outputs. The June 2026 completed results required about 14.15 non-overlapping compute hours on the tested CUDA setup after excluding a discarded overlapping cross-model attempt. The most expensive stages are systematic causal intervention, cross-model comparison, and strict all-layer CTCF scans, so the appendix includes both full reproduction commands and a reduced smoke-run command for checking the code path on a smaller subset.
...
\begin{table}[t]
\caption{Predictive task context before mechanistic analysis. GC-only and 3--6-mer rows are raw-sequence baselines. The frozen DNABERT-2 sequence head is trained on cached layer-11 sequence embeddings; the residual readout is reported separately as diagnostic decodability evidence.}
...
\midrule
promoter\_tata & 5,062 & 212 & 0.8955 & 0.9040 & 0.8019 & 0.9297 & 0.9378 & 0.8255 & 0.9137 & 0.9137 \\
promoter\_no\_tata & 30,000 & 1,372 & 0.9088 & 0.9218 & 0.8316 & 0.9406 & 0.9482 & 0.8652 & 0.9383 & 0.9383 \\
splice\_sites\_donors & 30,000 & 3,000 & 0.6560 & 0.6892 & 0.6013 & 0.8185 & 0.8093 & 0.7430 & 0.8954 & 0.8954 \\
splice\_sites\_acceptors & 30,000 & 3,000 & 0.6361 & 0.6343 & 0.5883 & 0.7956 & 0.7792 & 0.7220 & 0.8847 & 0.8847 \\
\bottomrule
\end{tabular}
}
\end{table}
...
\midrule
CTCF & 51,249-sequence strict scan; no classifier target & No CTCF residual classifier used as strict evidence & Best $r=0.3004$; fail & Best $\rho_h=1.3130$; fail & No candidate promoted after failed QK/enrichment & No aligned OV claim & Best cosine 0.1092; weak & Primary strict chain \\
Promoter-TATA & GC 0.8955; k-mer 0.9297; seq-head/readout 0.9137 AUROC & Decodable, but close to strong baselines & Not target-aligned in current artifact & Not target-aligned & Best $\mathrm{PM}=1.4029$; over-restores & L2H7 output cosine 0.1261; weak & Not promoter-specific & Auxiliary causal evidence \\
Splice donor & GC 0.6560; k-mer 0.8185; seq-head/readout 0.8954 AUROC & Decodable beyond GC and k-mer & Not target-aligned in current artifact & Not target-aligned & Best $\mathrm{PM}=0.1657$; below causal threshold & Not audited & Not splice-specific & Auxiliary negative-control evidence \\
\bottomrule
\end{tabular}
}
\end{table}
...
Table~\ref{tab:threshold-sensitivity} and Figure~\ref{fig:threshold-sensitivity} summarize threshold sweeps over QK correlation, matched enrichment, and auxiliary patching metrics. Relaxing the QK threshold alone yields 31 heads at $r\geq0.1$, five at $r\geq0.2$, one at $r\geq0.3$, and none at $r\geq0.4$. Relaxing matched enrichment alone yields three heads at $\rho_h\geq1.1$, one at $\rho_h\geq1.25$, and none at $\rho_h\geq1.5$. The strict CTCF chain remains negative: no head jointly passes QK and enrichment once $r\geq0.2$, even with $\rho_h\geq1.1$, and the most permissive joint count at $r\geq0.1,\rho_h\geq1.1$ is two heads, equal to the 95th percentile of a permuted-head alignment null. Token-level motif-score nulls provide an additional calibration: the observed support-token count is 256,918, compared with a shuffled-score 95th percentile of 36,093.05 and zero nearest-GC non-support tokens above the motif-support threshold. The promoter and splice patching thresholds are more permissive because those are auxiliary task-specific interventions, not CTCF motif-detector proof.

\begin{table}[t]
\caption{Threshold sensitivity summary. Counts are heads passing each single-metric threshold, except the joint row, which requires both CTCF QK and enrichment. Null references include permuted head alignment plus token-level motif-score and GC-matched background calibration.}
...
\midrule
CTCF & QK $r$ & 31 at $r\geq0.1$ & 0 at $r\geq0.5$ & 0.3004 & Motif-score shuffle: 256,918 observed vs 36,093.05 p95 \\
...
Splice donor & Patching $\mathrm{PM}$ & 0 at $\mathrm{PM}\geq0.25$ & 0 at $\mathrm{PM}\geq0.75$ & 0.1657 & Auxiliary task-specific negative result \\
\bottomrule
\end{tabular}
}
\end{table}
...
Splice-donor patching does not identify a threshold-crossing causal head in the full rerun & Patching should restore a corrupted splice-donor scalar target & Best mean $\mathrm{PM}=0.1657$ over 13,303 usable pairs & The splice-donor intervention is weak despite strong residual decodability & Effect is small on average and not tied to QK/enrichment motif evidence \\
...
Relaxed CTCF thresholds do not rescue the strict claim & Sensitivity sweep should reveal whether conclusions depend on the registered cutoffs & No joint QK/enrichment pass once $r\geq0.2$, even at $\rho_h\geq1.1$ & The negative CTCF conclusion is not an artifact of one conservative cutoff & Per-head shuffled QK-score nulls require raw per-token QK vectors \\
SAE and OV audits do not reveal a simple alternate CTCF circuit & Output-write or sparse features should align strongly with the biological target & TATA OV cosine 0.1261; CTCF SAE best cosine 0.1092 & Auxiliary audits remain consistent with a distributed or absent simple motif circuit & Audits are not exhaustive circuit discovery \\
\bottomrule
\end{tabular}
}
\end{table}
...
The completed June 2026 result set was reconstructed from two log files. The first run covered \texttt{write\_config} through \texttt{distributed\_feature\_search} from 13:37:04 to 23:26:04 local time on June 17, 2026. A first \texttt{cross\_model\_tokenization\_comparison} attempt then ran until 02:27:59 on June 18 and reached 49,000 of 51,249 DNABERT-2 CTCF sequences, but that work was discarded because the cross-model stage was rerun from the start. The completed cross-model rerun ran from 08:51:36 to 13:11:27 on June 18, 2026, and its manifest reports 15,591.074 seconds. Removing the discarded overlap gives 50,930.636 seconds of completed pipeline time, or 14h 08m 50.636s. The wall-clock span from first logged step to final completed rerun was 23h 34m 23s. The inspected result tree contains 136 files totaling about 5.06 GiB, including CSV/JSON summaries, figures, NPZ archives, and sparse-autoencoder checkpoints.
...
\item \path{results/pipeline_run_cross_model_tokenization_comparison.json}
...
Missing performance context & Added GC and 3--6-mer raw-sequence baselines plus a frozen DNABERT sequence head before mechanistic claims, while keeping frozen readouts labeled as residual decodability & Predictive performance context subsection & \path{results/tables/downstream_task_performance.csv} \\
